...
type UserStorageService interface {
	InitClient(cfg *config.Config) (*mongo.Client, error)
	Create(ctx context.Context, client *mongo.Client, msg *tgbotapi.Message, cfg *config.Config) error
	Update(ctx context.Context, client *mongo.Client, message *tgbotapi.Message, cfg *config.Config) error
...
func (us *userStorageService) InitClient(cfg *config.Config) (*mongo.Client, error) {
	client, err := us.storage.initMongoClient(cfg)
	if err != nil {
		return nil, err
	}
...
func (us *userStorageService) Create(ctx context.Context, client *mongo.Client, msg *tgbotapi.Message, cfg *config.Config) error {
	if err := us.storage.createUser(ctx, client, msg, cfg); err != nil {
		return err
	}
...
func (us *userStorageService) Update(ctx context.Context, client *mongo.Client, msg *tgbotapi.Message, cfg *config.Config) error {
	if err := us.storage.updateUser(ctx, client, msg, cfg); err != nil {
		return err
	}
